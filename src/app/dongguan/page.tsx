 ...
 *
 * - 旧路径 /dongguan 已通过 next.config.js redirects() 配置 301 跳转
 *   到新路径 /city/dongguan（SEO 友好，保留外链权重）
...
export default function DongguanDeprecatedPage() {
  const router = useRouter()
  useEffect(() => {
    router.replace('/city/dongguan', { scroll: false })
  }, [router])
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50">
      <div className="text-center">
        ...
        <p className="text-sm text-slate-500 mb-4">
          东莞站已迁移到 <code className="px-1.5 py-0.5 bg-slate-200 rounded">/city/dongguan</code>
        </p>

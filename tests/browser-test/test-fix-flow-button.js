// 良朋社 /guide/flow → /market/projects?recommend=flow Bug 修复验证
...
const BASE_URL = 'http://localhost:3001/guide/flow';
const TMP_DIR = process.platform === 'win32' ? path.join(os.tmpdir()) : '/tmp';
const REPORT_PATH = path.join(TMP_DIR, 'fix-test-report.json');
const SHOT_BEFORE = path.join(TMP_DIR, 'fix-test-before-click.png');
const SHOT_AFTER = path.join(TMP_DIR, 'fix-test-after-click.png');
...
(async () => {
  ...
  try {
    ...
    // ════════════════════════════════════════════════════════════════
    // 步骤 6: 找到"💪 我自己来，开始干！"按钮，截图保存
    // ════════════════════════════════════════════════════════════════
    console.log('[步骤 6] 定位按钮并截图');
    ...
    // ════════════════════════════════════════════════════════════════
    // 步骤 7: 在点击之前，验证 localStorage.opc_level 仍然是 'TRADER'
    // ════════════════════════════════════════════════════════════════
    ...
    // ════════════════════════════════════════════════════════════════
    // 步骤 10: 验证 URL：必须是 /market/projects?recommend=flow
    // ════════════════════════════════════════════════════════════════
    ...
    // ════════════════════════════════════════════════════════════════
    // 步骤 11: 验证 localStorage.opc_level 是否被同步更新为 'FLOW'
    // ════════════════════════════════════════════════════════════════
    ...
    const cardData = await page.evaluate(() => {
      ...
      out.first2 = cards.slice(0, 2).map((card, idx) => {
        ...
        const hasBorderBlue400 = card.className.includes('border-blue-400');
        // 检测 "优先推荐" 子节点
        ...
        return {
          ...
          badgeTag: badgeEl ? badgeEl.tagName : null,
          badgeClass: badgeEl ? badgeEl.className : null,
    ...
    cardData.first2.forEach((c) => {
      console.log(`  卡片${c.index + 1}: "${c.title}" | badge=${c.hasBadge} | ring2=${c.hasRing2} | borderBlue400=${c.hasBorderBlue400}`);
    });
    ...
    console.log('[截图 1] before click:', SHOT_BEFORE);
    console.log('[截图 2] after click :', SHOT_AFTER);
  } catch (err) {
